...
def fuse_conv_norm_relu(conv, norm, relu):
    fused_module = nn.Sequential(conv, norm, relu)
    return fused_module
...
class QuantizedBasicBlock(BasicBlock):
    ...
    def fuse_model(self):
         # Manually fuse Conv + BN + ReLU using the custom ConvBnReLU module
        self.conv1 = ConvBnReLU.from_existing(self.conv1, self.bn1, self.relu)
        self.conv2 = ConvBn.from_existing(self.conv2, self.bn2)
        
        self.bn1 = nn.Identity()
        self.relu = nn.Identity()
        self.bn2 = nn.Identity()
        
        if self.downsample is not None and isinstance(self.downsample, nn.Sequential) and len(self.downsample) == 2:
            self.downsample[0] = ConvBn.from_existing(self.downsample[0], self.downsample[1])
            self.downsample[1] = nn.Identity()
...
class QuantizedBottleneck(Bottleneck):
    ...
    def fuse_model(self):
        self.conv1 = ConvBnReLU.from_existing(self.conv1, self.bn1, self.relu)
        self.conv2 = ConvBn.from_existing(self.conv2, self.bn2, self.relu)
        self.conv3 = ConvBn.from_existing(self.conv3, self.bn3)
        ...
        # If downsample is present and has layers to fuse
        if self.downsample is not None and isinstance(self.downsample, nn.Sequential) and len(self.downsample) == 2:
            self.downsample[0] = ConvBn.from_existing(self.downsample[0], self.downsample[1])
            self.downsample[1] = nn.Identity()
...
class ConvBnReLU(nn.Module):
    ...
    @classmethod
    def from_existing(cls, conv, bn, relu):
        return cls(conv, bn, relu)
